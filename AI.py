...
def load_parameters(model):
    ...
    vram_gb = parse_memory(metadata.get("VRAM", "0GB"), "GB")
    ram_gb = parse_memory(metadata.get("RAM_available", "0MB"), "MB") / 1024
    ...
    if not arch:  error("unable to determine model architecture")

    arch = str(arch)
    layers = val(f"{arch}.block_count") or 0
    q_heads = val(f"{arch}.attention.head_count") or 0
    kv_heads = val(f"{arch}.attention.head_count_kv") or q_heads

    head_dim = (val(f"{arch}.attention.key_length") or val(f"{arch}.attention.head_dim") or 0)

    if not head_dim and q_heads:
        embed_len = val(f"{arch}.embedding_length")
        if embed_len:
            head_dim = embed_len // q_heads

    if not layers or not kv_heads or not head_dim:
        error("unable to determine model architecture parameters")

    n_threads = max(1, (os.cpu_count() or 1) - 1)

    if vram_gb > 0:
        available_vram = vram_gb * 1024 ** 3
        gpu_budget = available_vram * 0.85
        gpu_ratio = gpu_budget / model_size

        if gpu_ratio >= 1.0: gpu_layers = -1
        elif gpu_ratio <= 0: gpu_layers = 0
        else:
            gpu_layers = int(layers * gpu_ratio)
            gpu_layers = max(0, min(layers, gpu_layers))

    else:
        gpu_layers = 0

    if    gpu_layers == -1: cpu_model_size = 0
    elif  gpu_layers == 0: cpu_model_size = model_size
    else: cpu_model_size = model_size * (layers - gpu_layers) / layers

    available_ram = max(0, ram_gb * 1024 ** 3 - cpu_model_size)

    kv_bytes_per_token = (2 * layers * kv_heads * head_dim * 2)
    max_ctx = ( int(available_ram / kv_bytes_per_token) if kv_bytes_per_token else 2048)
    max_ctx = min(max_ctx, 131072)
    if max_ctx < 1: ctx = 256
    else:
